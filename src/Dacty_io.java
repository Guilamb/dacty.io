...
class Dacty_io extends Program {
	...
	String[] chercherCsv() {
		...
		for (int i = 0; i<length(ressources); i++) {
			if (length(ressources[i]) > 4 && charAt(ressources[i],length(ressources[i])-4) == '.' && charAt(ressources[i],length(ressources[i])-3) == 'c' && charAt(ressources[i],length(ressources[i])-2) == 's' && charAt(ressources[i],length(ressources[i])-1) == 'v') {
				nbCsv++;
			}
		}
		listeCsv = new String[nbCsv];
		while (cpt<nbCsv && cps<length(ressources)) {
			if (length(ressources[cps]) > 4 && charAt(ressources[cps],length(ressources[cps])-4) == '.' && charAt(ressources[cps],length(ressources[cps])-3) == 'c' && charAt(ressources[cps],length(ressources[cps])-2) == 's' && charAt(ressources[cps],length(ressources[cps])-1) == 'v') {
